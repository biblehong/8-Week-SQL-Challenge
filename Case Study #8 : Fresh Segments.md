...
We can see a decline in the overall trend of max avg composition further supported by the declining moving average to flatten the graph. This could indicate a potential problem in the business model. It is recommended to have the client review the customer interaction on a transaction level to find out the actual reason of the decline. One reason for the reduced customer interaction could be the outdated contents hence it might be worthwhile to add/update their assets with the newest contents to make them more engaging and interesting to customers. 
